...
import eu.nomad_lab.QueueMessage.{ToBeNormalizedQueueMessage, SingleParserQueueMessage}
...
object CalculationParserWorker extends  StrictLogging {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val message = SingleParserQueueMessage(
  ...
  def readFromQueue() = {
    ...
    val consumer: Consumer = new DefaultConsumer((channel)) {
      ...
      override def handleDelivery(consumerTag: String, envelope: Envelope, properties: AMQP.BasicProperties, body: Array[Byte]) {
        val message: SingleParserQueueMessage = eu.nomad_lab.JsonSupport.readUtf8[SingleParserQueueMessage](body)
