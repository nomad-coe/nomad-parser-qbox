...
class InfoKind(Base):
    ...
    def super_gids():
        ...
        def fget(self):
            res = map(lambda x: x.parent_gid, object_session(self).query(info_kind_parents.c.parent_gid).filter(info_kind_parents.c.info_kind_gid == self.gid).order_by(info_kind_parents.c.parent_gid).all())
                    ...
                    if knownIks:
                        if len(knownIks)>1:
                            logging.getLogger("nomad.InfoKind").warn(
                                    ...
                                    ikEl.kindStr, map(lambda x: x.toInfoKindEl().toDict(),knownIks),ikEl)
                        knownIks.sort()
...
class InfoUse(Base):
    ...
    uri = Column(String)
    uri_class = Column(Enum(*map(lambda x: x[1],storage_document.DocClass.choices), name='uri_classes'))
